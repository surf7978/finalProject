...
<div style="width:60%;">
	...
	<!-- 결제 및 배송  -->
		<input name="sumPrice" value="${vo.resultPrice }" type=""> 
		<input name="memberId" value="${loginID}" type="">  
		<input name="category1" type=""> 
	<c:forEach items="${list}" var="list" varStatus="num">
		...
		<c:if test="${pro.productNumber == list.productNumber}">
			<input name="buyList[${num2.index}].category" value="${pro.category }" type="">
		</c:if>
		</c:forEach>	
		<input name="buyList[${num.index}].fromPerson" value="${loginID}" type=""> 
		<input name="buyList[${num.index}].toPerson" value="${name }" type=""> 
		<input name="buyList[${num.index}].post" value="${bvo.post }" type="">
		<input name="buyList[${num.index}].address" value="${bvo.address }" type=""> 
		<input name="buyList[${num.index}].address2" value="${bvo.address2 }" type=""> 
		<input name="buyList[${num.index}].phone" value="${bvo.phone }" type=""> 
	<!-- 주소 end-->
		<input name="cartNumbers" value="${list.cartNumber}" type="hidden">
		<input name="buyList[${num.index}].productNumber" id="productNumber" value="${list.productNumber}" type="">
		<input name="buyList[${num.index}].optionName" value="${fn:replace(list.optionName,',','/') }" type="">
		<%-- <input name="buyList[${num.index}].price" value="${vo.resultPrice }"> --%>
		<input name="buyList[${num.index}].price" value="${fn:replace(list.optionPrice,',','/') }" type=""> 
		<input name="buyList[${num.index}].count" value="${fn:replace(list.count.trim(),',','/') }" type="">
	</div>
	</c:forEach>
	</form>
</div>
