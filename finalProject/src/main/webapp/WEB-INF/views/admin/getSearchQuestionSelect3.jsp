...
<script>
	function selChange() {
		...
		location.href="getSearchQuestionSelect3?nowPage=${paging.nowPage}&cntPerPage="+sel;
	}
</script>
<body>
<div style="width:60%;">
...
<tr onclick="location.href='getQuestion3?questionNumber=${list.questionNumber}'">
...
<td>${list.memberId} </td>
	...
	<div style="display: block; text-align: center;">		
		<c:if test="${paging.startPage != 1 }">
			<a href="getSearchQuestionSelect3?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
		</c:if>
		<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
			<c:choose>
				...
				<c:when test="${p != paging.nowPage }">
					<a href="getSearchQuestionSelect3?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
				</c:when>
			</c:choose>
		</c:forEach>
		<c:if test="${paging.endPage != paging.lastPage}">
			<a href="getSearchQuestionSelect3?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
		</c:if>
	</div>
</div>
</div>
